...
const Modal = forwardRef<ModalHandle, ModalProps>(function Modal(
  ...
  // useImperativeHandle is used to expose the `open` method to other component

  useImperativeHandle(ref, () => {
    return {
      open() {
        if (dialog.current) {
          dialog.current.showModal(); //ShowModal() is a built-in method avaiable on the <dialog> element.
        }
      }
    };
  });
